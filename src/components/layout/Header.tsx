...
const Header = ({ children }: IChildName) => {
  ...
  return (
    <header className="w-screen text-white justify-center items-center h-12 md:h-20 bg-black">
      <nav className="h-full  uppercase text-center flex justify-between mx-4 lg:mx-8">
        { router.pathname === '/' ? false : backBtn }
        <h1 className="flex self-center md:text-3xl text-center mx-auto text-base font-semibold">{ children }</h1>
